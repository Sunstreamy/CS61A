...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    t = [ps for ps in paragraphs if select(ps)]
    if k + 1 > len(t):
        return ""
    return t[k]
    # END PROBLEM 1
...
def about(subject):
    ...
    def select(text):
        ...
        if any(word in words for word in subject):
            return True
        return False
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    a, b = len(typed_words), len(source_words)
    if a == b == 0:
        return 100.0
    if a == 0:
        return 0.0
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        ...
        if min_diff > abs(diff):
            min_diff = abs(diff)
            ans = word
    return ans if min_diff <= limit else typed_word
    "*** YOUR CODE HERE ***"
    # END PROBLEM 5
...
def fastest_words(match):
    ...
    for word_index in word_indices:
        # Get times for each player for this word
        times = [
            time(match, player_index, word_index) for player_index in player_indices
        ]
        # 列表推导式是一种简洁的创建列表的方法
        # Find the fastest player (in case of tie, choose the first one)
