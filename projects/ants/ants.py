...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        list_bees = list(self.place.bees)  # 创建副本,确保能对所有蜜蜂造成伤害。
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt blocks the path to the next Place."""
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while temp_place is not None:
            ...
            elif temp_place.ant is not None:
                temp_place.ant.double()
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        if cur_place.ant is self and isinstance(cur_place.ant, ContainerAnt):
            insects[cur_place.ant] = dist
        for bee in list(cur_place.bees):
            insects[bee] = dist
        # 可攻击的位置（不在laser同一处)
        while cur_place.entrance and not cur_place.is_hive:
            cur_place = cur_place.entrance
            dist += 1
            if isinstance(cur_place.ant, ContainerAnt):
                insects[cur_place.ant] = dist
                if cur_place.ant.ant_contained is not None:
                    insects[cur_place.ant.ant_contained] = dist
            elif cur_place.ant is not None:
                insects[cur_place.ant] = dist
            for bee in list(cur_place.bees):
                insects[bee] = dist
        return insects
